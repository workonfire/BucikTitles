...
public class GUIManager {
    ...
    public static void showUserInterface(JavaPlugin plugin, Player player, int page) {
        /* Inicjalizacja GUI */
        Gui gui = new Gui(plugin, 6, ConfigManager.config.getString("gui.title"));
        gui.setOnGlobalClick(event -> event.setCancelled(true));
        gui.setOnOutsideClick(event -> event.setCancelled(true));

        /* Tworzenie obramowania */
        String outline_pane_material = ConfigManager.config.getString("gui.outline-pane.item");
        ItemStack outline_pane_item = new ItemStack(Material.getMaterial(outline_pane_material));
        ...
        if (ConfigManager.config.getString("gui.outline-pane.name") != null)
            outline_pane_item_meta.setDisplayName(ConfigManager.config.getString("gui.outline-pane.name"));
        if (ConfigManager.config.getStringList("gui.outline-pane.lore") != null)
            outline_pane_item_meta.setLore(ConfigManager.config.getStringList("gui.outline-pane.lore"));
        ...
        if (page != 1) {
            ...
            ItemStack back_button_item = new ItemStack(Material.BELL);
            ...
            if (ConfigManager.config.getString("gui.previous-page-button.name") != null)
                back_button_item_meta.setDisplayName(ConfigManager.config.getString("gui.previous-page-button.name"));
        ...
        if (ConfigManager.config.contains("titles.pages." + next_page)) {
            ...
            if (ConfigManager.config.getString("gui.next-page-button.name") != null)
                next_button_item_meta.setDisplayName(ConfigManager.config.getString("gui.next-page-button.name"));
        ...
        if (ConfigManager.config.getString("gui.clear-titles-button.name") != null)
            take_off_button_meta.setDisplayName(ConfigManager.config.getString("gui.clear-titles-button.name"));
        ...
        for (String title : ConfigManager.config.getConfigurationSection(format("titles.pages.%s", page)).getKeys(false)) {
            if (player.hasPermission(ConfigManager.config.getString(format("titles.pages.%s.%s.permission", page, title)))) {
                ...
                String title_item_name = ConfigManager.config.getString(format("titles.pages.%s.%s.gui-item.name", page, title));
                if (title_item_name != null)
                    title_item_meta.setDisplayName(title_item_name);
                if (ConfigManager.config.getStringList(format("titles.pages.%s.%s.gui-item.lore", page, title)) != null)
                    title_item_meta.setLore(ConfigManager.config.getStringList(format("titles.pages.%s.%s.gui-item.lore", page, title)));
