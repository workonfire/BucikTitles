        ...
        try {
            ...
            for (String title : ConfigManager.config.getConfigurationSection(format("titles.pages.%s", page)).getKeys(false)) {
                if (player.hasPermission(ConfigManager.config.getString(format("titles.pages.%s.%s.permission", page, title)))) {
                    ...
                    int title_item_amount = ConfigManager.config.getInt(format("titles.pages.%s.%s.gui-item.amount", page, title));
                    if (title_item_amount != 0) title_item.setAmount(title_item_amount);
            ...
            player.sendMessage(Main.prefix + Main.formatColors(ConfigManager.config.getString("language.config-load-error")));
