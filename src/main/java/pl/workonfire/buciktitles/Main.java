...
public final class Main extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        prefix = ConfigManager.config.getString("language.plugin-prefix");
    ...
    public static void setTitle(Player player, String title_id, int page) {
        ...
        player.sendMessage(prefix + ConfigManager.config.getString("language.title-set"));
    ...
    public static void takeOff(Player player) {
        ...
        player.closeInventory();
        player.sendMessage(prefix + ConfigManager.config.getString("language.title-removed"));
        ...
        if (args.length == 1) {
            if (args[0].equalsIgnoreCase("reload")) {
                if (sender.hasPermission("bucik.titles.reload")) {
                    ...
                    sender.sendMessage(prefix + ConfigManager.config.getString("language.plugin-reloaded"));
                }
                else sender.sendMessage(prefix + ConfigManager.config.getString("language.no-permission"));
            }
            ...
            else sender.sendMessage(prefix + ConfigManager.config.getString("language.subcommand-does-not-exist"));
        }
        else {
            if (sender.hasPermission("bucik.titles.open")) {
                if (sender instanceof Player) GUIManager.showUserInterface(plugin, (Player) sender, 1);
                else sender.sendMessage(prefix + ConfigManager.config.getString("language.cannot-open-from-console"));
            }
            else sender.sendMessage(prefix + ConfigManager.config.getString("language.no-permission"));
        }
        return true;
