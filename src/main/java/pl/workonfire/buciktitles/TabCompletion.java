...
public class TabCompletion implements TabCompleter {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (sender.hasPermission("bucik.titles.reload"))
            return (args.length > 0) ? StringUtil.copyPartialMatches(args[0], admin_commands, new ArrayList<>()) : null;
        else return (args.length > 0) ? StringUtil.copyPartialMatches(args[0], user_commands, new ArrayList<>()) : null;
    }
}
